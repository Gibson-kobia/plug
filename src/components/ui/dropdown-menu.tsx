...
>(({ className, sideOffset = 4, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        'z-50 min-w-[10rem] overflow-hidden rounded-md border border-neutral-200 bg-surface p-1 text-body shadow-xl',
        className
      )}
      {...props}
    />
...
>(({ className, children, checked, ...props }, ref) => (
  <DropdownMenuPrimitive.CheckboxItem
    ...
    className={cn(
      'relative flex cursor-pointer select-none items-center rounded-sm py-2 pl-8 pr-2 text-sm',
      'focus:bg-muted focus:outline-none data-[disabled]:opacity-60',
      className
    )}
    {...props}
  >
